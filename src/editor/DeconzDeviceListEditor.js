class DeconzDeviceListEditor extends DeconzEditor {
  ...
  async getItems(options, xhrParams) {
    ...
    if (result && result.error_message) {
      ...
      RED.notify(`Warning : ${result.error_message}`, {
        type: "warning",
        timeout: 10000,
      });
      return;
  ...
  generateHtmlItemList(items, htmlContainer) {
    ...
    for (const [group_key, item_list] of Object.entries(items).sort((a, b) => {
      // Sort by keys
      ...
      for (/** @type {Light | Sensor | Group} */ const item of item_list.sort(
        (a, b) => {
          // Sort by keys
        ...
        let opt = $("<option>" + label + "</option>").attr(
          "value",
          item.device_path
        );
