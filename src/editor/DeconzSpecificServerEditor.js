class DeconzSpecificServerEditor extends DeconzEditor {
  ...
  async discoverParams(overrideSettings) {
    ...
    myNotification = RED.notify(
      "<p>Trying to find the server settings, please wait...<br>" +
    ...
    try {
      ...
      params.discoverParam.devicetype =
        "Node-Red Deconz Plugin" + (this.node ? ` id:${this.node.id}` : "");
      let request = await $.getJSON(this.xhrURL, {
      ...
      if (request.error) {
        ...
        switch (request.error.code) {
          case "GATEWAY_CHOICE":
            html +=
              "<p>There is multiple Deconz device in you network, " +
            ...
            let clickMethod = function (gateway_id) {
              closeNotification();
              if (gateway_id) {
                request.currentSettings.discoverParam.targetGatewayID =
                  gateway_id;
              }
            ...
            for (const [
              index,
              gateway,
            ] of request.error.gateway_list.entries()) {
              buttons.push({
          ...
          case "DECONZ_ERROR":
            if (request.error.type === 101) {
              ...
              html +=
                "<p>The reason why the request failed is that the gateway was not unlocked. " +
        ...
        closeNotification();
        myNotification = RED.notify(
          `<p>Unknown error : ${JSON.stringify(request)}</p>`,
          {
            modal: true,
            fixed: true,
            type: "error",
      ...
      closeNotification();
      myNotification = RED.notify(
        `<p>Error while processing request: ${error.toString()}</p>`,
        {
          type: "error",
        }
      );
