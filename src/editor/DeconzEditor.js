...
class DeconzEditor {
  ...
  getI18n(prefix, suffix, data = {}) {
    ...
    data.defaultValue = "_deconz_undefined_";
    const value = RED._(_path, data);
    if (value === "_deconz_undefined_") return;
    return value;
  }
  ...
  async generateSimpleListField(container, options) {
    ...
    if (options.choices) {
      for (const [key, value] of options.choices) {
        input.append($("<option/>").attr("value", key).html(RED._(value)));
      }
    }
  ...
  async initTypedInput(input, options) {
    ...
    if (options.value) {
      if (options.value.type !== undefined)
        input.typedInput("type", options.value.type);
      if (options.value.value !== undefined)
        input.typedInput("value", options.value.value);
    }
  }
  ...
  async generateCheckboxField(container, options) {
    let input = $("<input/>", {
      ...
      type: "checkbox",
      style:
        "display: table-cell; width: 14px;vertical-align: top;margin-right: 5px",
      checked: options.currentValue,
    });

    let row = await this.generateInputWithLabel(input, options);
    row.append(
      $("<span/>").html(RED._(options.descText)).css("display", "table-cell")
    );
  ...
  // From https://stackoverflow.com/questions/6832596
