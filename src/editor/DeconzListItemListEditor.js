class DeconzListItemListEditor extends DeconzEditor {
  ...
  async initList(itemEditorClass, items = []) {
    ...
    this.$elements.list.editableList({
      ...
      addItem: (row, index, item) => {
        // Create item editor
        let itemEditor = new itemEditorClass(
          this.node,
          this,
          row,
          this.options
      ...
      removeItem: (item) => {
        if (item.uniqueId && this.items[item.uniqueId]) {
          ...
          throw new Error(
            `Error while removing the ${this.listType}, the ${this.listType} ${item.uniqueId} does not exist.`
          );
        }
      },
      sortItems: (items) => {
        ...
        items.each((index, item) => {
          if (this.items[item.attr("id")]) {
            ...
            throw new Error(
              `Error while moving the ${this.listType}, the ${this.listType} ${
                index + 1
              } does not exist.`
            );
          }
  ...
  get value() {
    let result = [];
    for (const rule of Object.values(this.items).sort(
      (a, b) => a.index - b.index
    )) {
