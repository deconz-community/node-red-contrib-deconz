class DeconzMainEditor extends DeconzEditor {
  constructor(node, options = {}) {
    ...
    if (this.options.have.statustext)
      this.subEditor.statustext = new DeconzStatusTextEditor(
        this.node,
        this.options.statustext
      );
    // TODO pourquoi le device dois être init avant le query ???
    if (this.options.have.device)
      this.subEditor.device = new DeconzDeviceEditor(
        this.node,
        this.options.device
      );
    if (this.options.have.query)
      this.subEditor.query = new DeconzQueryEditor(
        this.node,
        this.options.query
      );
    if (this.options.have.specific) {
      switch (this.node.type) {
        case "deconz-api":
          this.subEditor.specific = new DeconzSpecificApiEditor(
            this.node,
            this.options.specific.api
          );
          break;
        case "deconz-output":
          this.subEditor.specific = new DeconzSpecificOutputEditor(
            this.node,
            this.options.specific.output
          );
          break;
        case "deconz-server":
          this.subEditor.specific = new DeconzSpecificServerEditor(
            this.node,
            this.options.specific.server
          );
          break;
      }
    }
    if (this.options.have.output_rules)
      this.subEditor.output_rules = new DeconzOutputRuleListEditor(
        this.node,
        this.options.output_rules
      );
    if (this.options.have.commands)
      this.subEditor.commands = new DeconzCommandListEditor(
        this.node,
  ...
  async configurationMigration() {
    ...
    let errorMsg =
      "Error while migrating the configuration of the node from version " +
      (this.node.config_version || 0) +
      " to version " +
      this.node._def.defaults.config_version.value +
      ".";
    ...
    let result = await $.getJSON(
      `${this.NRCD}/configurationMigration`,
      data
    ).catch((t, u) => {
    ...
    let mapI18N = (msg) =>
      msg.substr(0, 23) === "node-red-contrib-deconz" ? RED._(msg) : msg;
  ...
  get serverNode() {
    return this.node.type === "deconz-server"
      ? this.node
      : RED.nodes.node(this.$elements.server.val());
  }
