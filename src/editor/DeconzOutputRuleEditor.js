class DeconzOutputRuleEditor extends DeconzListItemEditor {
  ...
  async init(rule, index) {
    ...
    if (this.node.type === "deconz-input")
      await this.generateOutputField(
        this.container,
        rule.output !== undefined ? rule.output : this.defaultRule.output
      );
    if (["deconz-input", "deconz-battery"].includes(this.node.type))
      await this.generateOnStartField(
        this.container,
        rule.onstart !== undefined ? rule.onstart : this.defaultRule.onstart
      );
    if (this.node.type === "deconz-input")
      await this.generateOnErrorField(
        this.container,
        rule.onerror !== undefined ? rule.onerror : this.defaultRule.onerror
      );
  ...
  async connect() {
    ...
    this.$elements.type.on("change", () => {
      ...
      if (["attribute", "state", "config", "homekit"].includes(type))
    ...
    this.$elements.outputButton.on("click", () => {
      try {
        let nodes = RED.nodes
          .filterLinks({ source: this.node, sourcePort: this.index })
          .map((l) => {
        ...
        let myNotification = RED.notify(
          `The output ${this.index + 1} is sending message to ${
            nodes.length
          } nodes :<br>${nodes.join("<br>")}`,
          {
            modal: true,
            timeout: 5000,
            buttons: [
        ...
        this.sendError(
          `This is using not documented API so can be broken at anytime.<br>Error while getting connected nodes: ${e.toString()}`
        );
      }
    });
  }
  ...
  async updatePayloadList() {
    ...
    if (type === "homekit") {
      html +=
        ...
        RED._(`${i18n}.options.auto`) +
        "</option>";
    } else {
      html +=
        '<option value="__complete__">' +
        RED._(`${i18n}.options.complete`) +
        "</option>";
      if (this.options.enableEachState === true) {
        html +=
          '<option value="__each__">' +
          RED._(`${i18n}.options.each`) +
          "</option>";
      }
    }
    ...
    if (queryType === "device") {
      let data = await $.getJSON(`${this.NRCD}/${type}list`, {
        ...
        devices: JSON.stringify(
          this.listEditor.mainEditor.subEditor.device.value
        ),
      });

      let type_list =
        type === "attribute" ? ["attribute", "state", "config"] : [type];
      ...
      for (const _type of type_list) {
        ...
        for (const item of Object.keys(data.count[_type]).sort()) {
          ...
          $("<option>" + label + "</option>")
            .attr(
              "value",
              type === "attribute" && _type !== "attribute"
                ? `${_type}.${item}`
                : item
            )
            .appendTo(groupHtml);
  ...
  async initPayloadList(value) {
    ...
    list.multipleSelect({
      ...
      filter: true,
      filterPlaceholder: RED._(
        `${this.NRCD}/server:editor.inputs.device.device.filter`
      ),
      placeholder: RED._(
  ...
  async generatePayloadFormatField(container, value) {
    ...
    let choices = [];
    for (const [format, enabled] of Object.entries(
      this.listEditor.options.format
    )) {
      if (enabled) {
