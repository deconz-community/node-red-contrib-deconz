class DeconzCommandEditor extends DeconzListItemEditor {
  ...
  async init(command, index) {
    ...
    await this.generateTypeDomainField(this.container, {
      type: command.type,
    ...
    for (const lightType of ["bri", "sat", "hue", "ct", "xy"]) {
      await this.generateLightColorField(
        this.containers.light,
        lightType,
        command.arg[lightType]
      );
      if (lightType === "bri") await this.generateHR(this.containers.light);
    }
    await this.generateHR(this.containers.light);
    await this.generateLightAlertField(
      this.containers.light,
      command.arg.alert
    );
    await this.generateLightEffectField(
      this.containers.light,
      command.arg.effect
    );
    await this.generateLightColorLoopSpeedField(
      this.containers.light,
    ...
    await this.generateCoverOpenField(
      this.containers.windows_cover,
      command.arg.open
    );
    await this.generateCoverStopField(
      this.containers.windows_cover,
      command.arg.stop
    );
    await this.generateCoverLiftField(
      this.containers.windows_cover,
      command.arg.lift
    );
    await this.generateCoverTiltField(
      this.containers.windows_cover,
      command.arg.tilt
    );
    ...
    await this.generateSceneModeField(
      this.containers.scene_call,
      command.arg.scene_mode
    );
    this.containers.scene_call_single = $("<div>").appendTo(
      this.containers.scene_call
    );
    await this.generateScenePickerField(
      this.containers.scene_call_single,
      `${command.arg.group}.${command.arg.scene}`
    );
    await this.generateSceneGroupField(
      this.containers.scene_call_single,
      command.arg.group
    );
    await this.generateSceneSceneField(
      this.containers.scene_call_single,
      command.arg.scene
    );
    this.containers.scene_call_dynamic = $("<div>").appendTo(
      this.containers.scene_call
    );
    await this.generateSceneNameField(
      this.containers.scene_call_dynamic,
      command.arg.scene_name
    );
    ...
    await this.generateCommandField(
      this.containers.command,
      command.arg.command
    );

    // Payload
    this.containers.payload = $("<div>").appendTo(this.container);
    await this.generatePayloadField(
      this.containers.payload,
      command.arg.payload
    );

    // Pause
    this.containers.pause = $("<div>").appendTo(this.container);
    await this.generatePauseDelayField(
    ...
    await this.generateCommonTransitionTimeField(
      this.containers.transition,
      command.arg.transitiontime
    );

    this.containers.common = $("<div>").appendTo(this.container);
    await this.generateHR(this.containers.common);
    await this.generateCommonOnErrorRetryField(
      this.containers.common,
      command.arg.retryonerror
    );
    await this.generateCommonOnErrorAfterField(
      this.containers.common,
  ...
  async connect() {
    ...
    this.$elements.outputButton.on("click", async () => {
      try {
        ...
        if (queryMode !== "device") {
          this.sendError(
            "Error : The run command can only work with device list.",
            5000
          );
          return;
        }
        ...
        if (
          ...
          !(
            ...
            (command.type === "custom" &&
              command.arg.target.type === "scene_call")
          )
        ) {
        ...
        for (const [name, value] of Object.entries(command.arg)) {
          if (["msg", "flow", "global", "jsonata"].includes(value.type)) {
            this.sendError(
              `Error : Cant run this command because the value "${name}" is type "${value.type}".`,
              5000
            );
            return;
          }
        ...
        let result = await $.post(`${this.NRCD}/testCommand`, {
          ...
          this.sendError(
            t.status === 400 && t.responseText ? t.responseText : u.toString()
          );
        });
    ...
    const updateScenePickerSelection = () => {
      ...
      this.$elements.scene_picker.multipleSelect(
        "setSelects",
        this.$elements.group.typedInput("type") !== "num" ||
          this.$elements.group.typedInput("type") !== "num"
          ? []
          : [
              `${this.$elements.group.typedInput(
                "value"
              )}.${this.$elements.scene.typedInput("value")}`,
            ]
      );
    ...
    const updateSceneMode = () => {
      const isSingle =
        this.$elements.scene_mode.typedInput("type") === "single";
    ...
    this.$elements.group.on("change", updateScenePickerSelection);
    this.$elements.scene.on("change", updateScenePickerSelection);
    this.$elements.scene_picker_refresh.on("click", () =>
  ...
  async updateShowHide(type, domain) {
    ...
    switch (type) {
      ...
      case "custom":
        ...
        if (this.$elements.target.typedInput("type") !== "scene_call")
  ...
  async updateSceneList() {
    ...
    let groups = await queryEditor.getItems(
      { refresh: true, keepOnlyMatched: true },
      params
    );
    ...
    for (const group of groups.LightGroup) {
      ...
      if (group.scenes && group.scenes.length > 0) {
        for (const scene of group.scenes) {
          $(`<option>${scene.id} - ${scene.name}</option>`)
            .attr("value", `${group.id}.${scene.id}`)
            .appendTo(groupHtml);
        }
  ...
  async generateLightColorField(container, fieldName, value = {}) {
    ...
    let directionsFormat = [
      this.generateTypedInputType(`${i18n}.lightFields`, "set", {
        hasValue: false,
      }),
    ];
    ...
    switch (fieldName) {
      case "bri":
        ...
        directionsFormat.push(
          this.generateTypedInputType(`${i18n}.lightFields`, "inc", {
            hasValue: false,
          })
        );
        directionsFormat.push(
          this.generateTypedInputType(`${i18n}.lightFields`, "dec", {
            hasValue: false,
          })
        );
        directionsFormat.push(
      ...
      case "ct":
        fieldFormat.push("str");
        fieldFormat.push(
          this.generateTypedInputType(`${i18n}.ct`, "deconz", {
    ...
    await this.generateDoubleTypedInputField(
      container,
      {
      ...
      typedInput: {
        types: [
          ...
          this.generateTypedInputType(i18n, "deconz", {
            subOptions: ["none", "select", "lselect"],
          }),
        ],
      },
    });
  ...
  async generateScenePickerField(container, value = "") {
    ...
    list.multipleSelect({
      ...
      placeholder: RED._(
        `${this.NRCD}/server:editor.multiselect.none_selected`
      ),
