      ...
      switch (typeof value[side]) {
        case "string":
          ...
          if (Number.isNaN(result)) {
            throw Error(
              `Invalid value provided for date comparaison, can't parse '${typeof value[side]}'`
            );
          } else {
            this[side] = result;
            this.valid = true;
        ...
        default:
          throw Error(
            `Invalid value type provided for date comparaison, got '${typeof value[side]}' and expect 'string' or 'number'`
          );
      }
    }
  }
